...
# Principales slots comerciales (sin fecha_mudanza - flujo simplificado)
COMMERCIAL_SLOT_PRIORITY = ("zona", "tipologia", "presupuesto")


# Regex helpers
CURRENCY_PATTERN = (
    r"(?:usd|u\$s|us\$|ars|\$|pesos|peso|argentinos|arg|dolares|dolar|dólares|dólar|us)"
)
...
def _build_valid_zona_lookup() -> dict[str, str]:
    lookup: dict[str, str] = {}
    for zone in NEIGHBORHOOD_GAZETTEER:
        lookup[_normalize_text(zone)] = zone
    for alias, proper in CITY_ALIASES.items():
        lookup[_normalize_text(alias)] = proper
    return lookup


def normalize_known_zona(value: str | None) -> str | None:
    if not value:
        return None
    norm = _normalize_text(value)
    if not norm:
        return None
    lookup = _build_valid_zona_lookup()
    if norm in lookup:
        return lookup[norm]
    return None


def is_valid_zona(value: str | None) -> bool:
    return normalize_known_zona(value) is not None
...
def parse_zona(text: str) -> str | None:
    ...
    match = re.search(
        r"\b(?:en|zona|barrio|por)\s+([a-z\s]+?)(?:$|[.,;]|\s(?:y|con|para)\s)", norm
    )
    if match:
        extracted = match.group(1).strip()
        if len(extracted) > 2:
            candidate = _title_case_zona(extracted)
            canonical = normalize_known_zona(candidate)
            if canonical:
                return canonical
...
def _normalize_answered_slots(answered_fields: set[str] | list[str] | None) -> set[str]:
...
def _extract_ambientes_count(tipologia: str | None) -> int | None:
    """Extract numeric ambientes count from tipologia string (e.g., '3 ambientes' -> 3)."""
    if not tipologia:
        return None
    match = re.search(r"(\d+)", str(tipologia))
    if match:
        try:
            return int(match.group(1))
        except ValueError:
            pass
    if "monoambiente" in str(tipologia).lower():
        return 1
    return None


def build_next_best_question(
    missing: list[str], current_values: dict[str, Any] | None = None
) -> tuple[str | None, str | None]:
    """
    Returns (question_text, key_for_that_question).
    Strict Slot Order: zona+ambientes -> presupuesto (mudanza removed - flujo simplificado)

    NEW: Handles partial zona/ambientes with human-like acknowledgment:
    - If zona present but ambientes missing: "Perfecto, zona {ZONA}. ¿Cuántos ambientes necesitás?"
    - If ambientes present but zona missing: "Perfecto, {AMBIENTES} ambientes. ¿Por qué zona buscás?"
    - If both missing: "¿Por qué zona buscás y cuántos ambientes necesitás?"
    """
    if not missing:
        return None, None
    ...
    # 1) Zona + Ambientes (handle partial capture with human-like responses)
    zona_missing = "zona" in m_set
    tipologia_missing = "tipologia" in m_set
    zona_value = current.get("zona")
    tipologia_value = current.get("tipologia")

    if zona_missing and tipologia_missing:
        # Both missing - ask combined question
        return "¿Por qué zona buscás y cuántos ambientes necesitás?", "zona"
    elif zona_missing and not tipologia_missing and tipologia_value:
        # Have ambientes, missing zona - acknowledge and ask for zona
        ambientes_count = _extract_ambientes_count(tipologia_value)
        if ambientes_count:
            return (
                f"Perfecto, {ambientes_count} ambientes. ¿Por qué zona buscás?",
                "zona",
            )
        else:
            return f"Perfecto, {tipologia_value}. ¿Por qué zona buscás?", "zona"
    elif tipologia_missing and not zona_missing and zona_value:
        # Have zona, missing ambientes - acknowledge and ask for ambientes
        return (
            f"Perfecto, zona {zona_value}. ¿Cuántos ambientes necesitás?",
            "tipologia",
        )
    elif zona_missing:
        # Only zona missing (tipologia not captured yet but not in missing set - edge case)
        return "¿En qué zona o barrio estás buscando?", "zona"
    elif tipologia_missing:
        # Only tipologia missing
        return "¿Qué tipología buscás? (Ej: 2 ambientes, monoambiente)", "tipologia"

    # 2) Presupuesto (last question before handoff)
...
def get_next_question_with_anti_repetition(
    ...
    answered_fields: set[str] | list[str] | None = None,
) -> tuple[str | None, str | None]:
    """
    Returns: (question_text, question_key)
    """
    missing = calculate_missing_slots(memory, answered_fields=answered_fields)
    if not missing:
        # All complete
